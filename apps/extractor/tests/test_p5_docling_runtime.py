...
def test_real_docling_provider_preserves_native_html_structure() -> None:
    ...
    source = repository_root / "test" / "fixtures" / "document-intelligence" / "complex-layout.html"
    ...
    assert any("Signal" in row for table in artifact.tables for row in table.rows)
