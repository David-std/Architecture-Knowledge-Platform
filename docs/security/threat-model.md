...
| Local file disclosure              | ingest/project scan           | canonical real path, registered allowlist roots, file-only/immutable-commit checks                       | wider configured roots increase the operator-approved access surface |
...
| Malformed identifier/error leakage | API/DB                        | UUID validation returns structured HTTP 400 without database detail                                      | maintained authorization/error tests must be rerun after changes     |
...
| Dependency/image drift             | supply chain                  | lockfiles, frozen installs, audit/peer gates and pinned runtime/model revisions                           | CI actions and container base images are not all digest/SHA pinned   |
